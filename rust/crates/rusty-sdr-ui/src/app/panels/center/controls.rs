...
impl SdrApp {
    ...
    ///
    /// Single scrollable row, groups ordered by change frequency:
    /// DEMOD | ZOOM | REF | RX (Signal + VU) | WF (Speed + Palette) | FFT | BP/PH
    ///
    ...
    pub(in crate::app) fn show_display_controls(
        ...
        ui.horizontal(|ui| {
            // ── Group A: DEMOD mode ──────────────────────────────────────────
            // Primary receiver control — first so it's always visible.
            ...
            // ── Group D: RX monitoring — signal quality bar + VU meter ───────
            // Both are passive live indicators; no internal separator between them.
            {
                ...
                for i in 0..5_u8 {
                    ...
                    painter.rect_filled(
                        seg_rect,
                        0.5,
                        if i < segments_lit { bar_color } else { egui::Color32::from_gray(50) },
                    );
                }
                ui.add_sized(
                    [60.0, 14.0],
            ...
            // ── Group E: WF — waterfall speed + palette ──────────────────────
            // Both controls are waterfall-specific; combined under one "WF" label.
            ...
            egui::ComboBox::from_id_salt("fft_window")
                ...
                .width(68.0)
                .show_ui(ui, |ui| {
                    use rusty_sdr_core::dsp::FftWindow;
                    for wf in [
