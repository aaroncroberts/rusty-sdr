...
impl SatMapWindow {
    ...
    pub fn show(&mut self, ctx: &egui::Context, open: &mut bool, home_lat: f64, home_lon: f64) {
        ...
        for (i, tle) in self.tles.iter().enumerate() {
            ...
            sd.pos = rusty_sdr_tle::propagator::position_at(tle, now, home_lat, home_lon);
        }
    ...
    fn draw_map(
        ...
        for (i, (tle, sd)) in self.tles.iter().zip(self.sat_data.iter()).enumerate() {
            ...
            let (dot_r, fill_color, stroke_color) = if is_flashing {
                ...
                (3.0, Color32::from_rgba_premultiplied(60, 80, 60, 160), Color32::from_rgba_premultiplied(80, 100, 80, 100))
            };

            // Selection ring behind the dot
            if is_selected {
                painter.circle_stroke(screen, dot_r + 4.0, Stroke::new(1.5, Color32::from_rgb(0x4E, 0xC9, 0xE0)));
            }
            painter.circle_filled(screen, dot_r, fill_color);
            painter.circle_stroke(screen, dot_r, Stroke::new(1.0, stroke_color));
            ...
            // Hover tooltip — show name + elevation for any satellite under the cursor
            if let Some(hover_pos) = response.hover_pos() {
                if hover_pos.distance(screen) < dot_r + 8.0 {
                    let status_str = if heard { "Contact established" }
                                     else if above_horizon { "Above horizon" }
                                     else { "Below horizon" };
                    response.clone().on_hover_ui_at_pointer(|ui| {
                        ui.label(RichText::new(tle.name.trim()).strong().color(fill_color));
                        ui.label(RichText::new(format!("El {:.1}°  Az {:.0}°", pos.el_deg, pos.az_deg)).small());
                        ui.label(RichText::new(status_str).small().color(
                            if heard { Color32::from_rgb(0x44, 0xFF, 0xEE) }
                            else if above_horizon { Color32::from_rgb(0x88, 0xFF, 0x88) }
                            else { Color32::GRAY }
                        ));
                    });
                }
            ...
            if response.clicked() {
                if let Some(click_pos) = response.interact_pointer_pos() {
                    if click_pos.distance(screen) < dot_r + 6.0 {
                        self.selected_idx = Some(i);
                    }
                }
            }
        }
        // Accumulate decoded satellites into persistent heard set, then clear flash.
        for norad_id in &self.flash_norad_ids {
            self.heard_norad_ids.insert(*norad_id);
        }
    ...
    fn draw_sidebar(&mut self, ui: &mut egui::Ui, panel_rect: Rect) {
        ...
        // Satellite list — sorted: heard first, then above horizon, then below.
        // Painter-drawn dot indicators avoid font glyph coverage issues.
        let mut sat_indices: Vec<usize> = (0..self.tles.len()).collect();
        sat_indices.sort_by_key(|&i| {
            let heard = if self.heard_norad_ids.contains(&self.tles[i].norad_id) { 0u8 } else { 1 };
            let above = if self.sat_data[i].pos.as_ref().map(|p| p.el_deg > 0.0).unwrap_or(false) { 0u8 } else { 1 };
            (heard, above)
        });

        let n_heard = sat_indices.iter().filter(|&&i| self.heard_norad_ids.contains(&self.tles[i].norad_id)).count();
        let n_above = sat_indices.iter().filter(|&&i| {
            !self.heard_norad_ids.contains(&self.tles[i].norad_id)
            && self.sat_data[i].pos.as_ref().map(|p| p.el_deg > 0.0).unwrap_or(false)
        }).count();

        if n_heard > 0 {
            sidebar_ui.label(RichText::new("Contact").size(9.0).color(Color32::from_rgb(0x22, 0xBB, 0xAA)));
        } else {
            sidebar_ui.label(RichText::new("Satellites").small().color(Color32::GRAY));
        }
        egui::ScrollArea::vertical()
            .max_height(panel_rect.height() * 0.38)
            .id_salt("sat_list")
            .show(&mut sidebar_ui, |ui| {
                ...
                for &i in &sat_indices {
                    ...
                    let text_color = if is_selected { Color32::WHITE }
                                     else if heard { Color32::from_rgb(0x88, 0xEE, 0xDD) }
                                     else if above { Color32::from_rgb(0xAA, 0xDD, 0xAA) }
                                     else { Color32::from_rgb(0x55, 0x65, 0x55) };

                    let row_resp = ui.horizontal(|ui| {
                        ui.spacing_mut().item_spacing.x = 4.0;
                        let (dot_rect, dot_resp) = ui.allocate_exact_size(Vec2::new(10.0, 13.0), Sense::click());
                        let c = dot_rect.center();
                        if heard {
                            ui.painter().circle_filled(c, 3.5, dot_color);
                            ui.painter().circle_stroke(c, 3.5, Stroke::new(1.0, Color32::from_rgb(0x44, 0xFF, 0xEE)));
                        } else if above {
                            ui.painter().circle_filled(c, 3.0, dot_color);
                        } else {
        ...
        if let Some(idx) = self.selected_idx {
            if let (Some(tle), Some(sd)) = (self.tles.get(idx), self.sat_data.get(idx)) {
                ...
                sidebar_ui.horizontal(|ui| {
                    let (badge_text, badge_color) = if heard {
                        ("CONTACT", Color32::from_rgb(0x22, 0xBB, 0xAA))
                    } else if above {
                        ("VISIBLE", Color32::from_rgb(0x55, 0xCC, 0x55))
                    } else {
                        ("BELOW HORIZON", Color32::from_rgb(0x55, 0x55, 0x55))
                    };
                    ui.label(RichText::new(badge_text).size(9.0).strong().color(badge_color));
                    if let Some(ref pos) = sd.pos {
                        ui.label(RichText::new(format!("  El {:.1}°", pos.el_deg)).size(9.0).color(Color32::GRAY));
                    }
                });
