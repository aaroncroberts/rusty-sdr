...
impl SdrApp {
    ...
    pub(in crate::app) fn device_settings_section(
        ...
        if !is_demo {
            let (status, serial, hw_ver, api_ver, error_count, error_log) = {
                let s = self.shared.read();
            ...
            .show(ui, |ui| {
                ...
                let status_color = if status.starts_with("Running") {
                    theme::STATUS_OK
                } else if status.starts_with("Reconnecting") || status.starts_with("Connecting") {
                    theme::AMBER
                } else {
                    theme::DANGER
                };
                ui.horizontal(|ui| {
                    ...
                    ui.label(
                        RichText::new(if status.is_empty() { "—" } else { &status })
                            .color(status_color)
                            .small(),
                    );
                });
                ...
                if !serial.is_empty() {
                    ui.horizontal(|ui| {
                        ui.label(RichText::new("Serial").color(theme::TEXT_MUTED).small());
                        ui.label(RichText::new(&serial).small());
                    });
                    ui.horizontal(|ui| {
                        ui.label(RichText::new("HW ver").color(theme::TEXT_MUTED).small());
                        ui.label(RichText::new(format!("{hw_ver}")).small());
                    });
                }
                if !api_ver.is_empty() {
                    ui.horizontal(|ui| {
                        ui.label(RichText::new("API ver").color(theme::TEXT_MUTED).small());
                        ui.label(RichText::new(&api_ver).small());
                    });
                }

                // Error count
                let err_color = if error_count > 0 { theme::DANGER } else { theme::TEXT_MUTED };
                ui.horizontal(|ui| {
                    ui.label(RichText::new("Errors").color(theme::TEXT_MUTED).small());
                    ui.label(RichText::new(format!("{error_count}")).color(err_color).small());
                });

                // Error log
                if !error_log.is_empty() {
                    ui.add_space(2.0);
                    ui.label(RichText::new("Error log (recent)").color(theme::TEXT_MUTED).small());
                    egui::ScrollArea::vertical()
                        .id_salt("device_error_log")
                        .max_height(80.0)
                        .show(ui, |ui| {
                            for entry in error_log.iter().rev() {
                                // Format timestamp as HH:MM:SS (local offset from Unix epoch).
                                let secs = entry.timestamp_secs % 86400;
                                let hh = secs / 3600;
                                let mm = (secs % 3600) / 60;
                                let ss = secs % 60;
                                let ts = format!("{hh:02}:{mm:02}:{ss:02}");
                                ui.label(
                                    RichText::new(format!("[{ts}] {}", entry.message))
                                        .color(theme::DANGER)
                                        .small(),
                                );
                            }
                    ...
                    if ui
                        .button(RichText::new("Copy diagnostics").small())
                        .on_hover_text("Copy device info and error log to clipboard")
                        .clicked()
                    {
                        let mut text = format!(
                            "SDRplay RSPdx-R2 Diagnostics\nStatus: {status}\n\
                             Serial: {serial}\nHW ver: {hw_ver}\nAPI ver: {api_ver}\n\
