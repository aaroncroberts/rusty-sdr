...
impl SdrApp {
    ...
    pub(in crate::app) fn show_display_controls(
        ...
        // MIDI Learn state (read before layout to avoid split-borrow issues)
        ...
        // Action flags: set by context_menu closures, acted on after columns return
        ...
        // Read FFT state before the layout closure
        ...
        // ── Four-column layout ────────────────────────────────────────────────
        // Columns: [0] REF LEVEL  [1] ZOOM  [2] WF / FFT  [3] DISPLAY
        ui.columns(4, |cols| {
            // ── Column 0 : REF LEVEL ─────────────────────────────────────────
            cols[0].vertical(|ui| {
                ui.label(RichText::new("REF LEVEL").color(theme::TEXT_MUTED).small());

                // A*/A toggle + Auto snap — horizontal pair
                ui.horizontal(|ui| {
                    let auto_color = if self.auto_ref { theme::ACCENT } else { theme::TEXT_MUTED };
                    if ui
                        .small_button(
                            RichText::new(if self.auto_ref { "A*" } else { "A" })
                                .color(auto_color),
                        )
                        .on_hover_text(
                            "Auto ref: tracks signal ceiling automatically. Click to toggle.",
                        )
                        .clicked()
                    {
                    ...
                    if ui
                        ...
                        .on_hover_text(
                            "Snap to current signal level; click again to keep tracking.",
                        )
                        .clicked()
                    {
                        if !self.auto_ref && self.noise_floor_ema > -120.0 {
                            self.fft_floor =
                                (self.noise_floor_ema - 5.0).clamp(-160.0, -10.0);
                            self.fft_ceil = (self.signal_ceil_ema + 5.0)
                                .clamp(self.fft_floor + 10.0, 0.0);
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui
                        ...
                        .on_hover_text(
                            "Spectrum/waterfall ceiling (dBFS). Drag to zoom in on weaker signals.",
                        )
                        .changed()
                    {
                ...
                ui.horizontal(|ui| {
                    ...
                    if ui
                        ...
                        .on_hover_text(
                            "Spectrum/waterfall floor (dBFS). Drag down to reveal weaker signals.",
                        )
                        .changed()
                    {
            ...
            cols[1].vertical_centered(|ui| {
                ...
                let zoom_resp = KnobWidget {
                    ...
                    step: 0.05,
                    diameter: 40.0,
                    label: None, // label already shown above
                    unit: "",
                    midi_cc: zoom_cc,
            ...
            // ── Column 2 : WATERFALL / FFT ───────────────────────────────────
            cols[2].vertical_centered(|ui| {
                ui.label(RichText::new("WF / FFT").color(theme::TEXT_MUTED).small());
                        ...
                        .show_ui(ui, |ui| {
                            for sz in [512_usize, 1024, 2048, 4096, 8192] {
                                ...
                                if ui.selectable_label(sel, sz.to_string()).clicked() && !sel {
                                    let _ = self
                                        .cmd_tx
                                        .try_send(DisplayCmd::SetFftSize(sz).into());
            ...
            cols[3].vertical_centered(|ui| {
                ui.label(RichText::new("DISPLAY").color(theme::TEXT_MUTED).small());

                // Band plan + Peak hold toggles
                ui.horizontal(|ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.add_sized(
                        [60.0, 14.0],
                        egui::Label::new(
                            RichText::new(quality_label).color(bar_color).small(),
                        ),
                    );
                ...
                ui.horizontal(|ui| {
                            ...
                            for name in ["Thermal", "Inferno", "Grayscale", "Classic"] {
                                ...
                                if ui.selectable_label(sel, name).clicked() && !sel {
                                    let cm: crate::theme::WaterfallColormap = match name {
                                        "Grayscale" => {
                                            crate::theme::WaterfallColormap::Grayscale
                                        }
                    ...
                    for i in 0..seg_count {
                        let y = vu_rect.bottom()
                            - (i as f32 + 1.0) * (seg_h + seg_gap)
                            + seg_gap;
                        let seg_rect = egui::Rect::from_min_size(
        ...
        // Handle MIDI Learn actions deferred from the columns closure
        if zoom_learn_req {
